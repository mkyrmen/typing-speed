...
function Stat({ label, value, flash, icon }: { label: string; value: React.ReactNode; flash: boolean; icon: string }) {
  return (
    <div className="flex items-baseline gap-2">
      <span className="text-[11px] tracking-[0.28em] uppercase opacity-40 flex items-center gap-1">
        <span className="text-sm pb-[2px]">{icon}</span>
        {label}
      </span>
      <span
        className={[
          "tabular-nums font-bold text-xl transition-all duration-200",
